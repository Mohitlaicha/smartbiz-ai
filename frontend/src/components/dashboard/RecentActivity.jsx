...
export default function RecentActivity({
  ...
  const activities = [
    ...invoices.slice(0, 3).map((i) => ({
      type: "invoice",
      title: `Invoice ${i.invoice_number || i.invoiceNumber || i.id}`,
      subtitle: `${i.customer_name || i.customerName || "Customer"} — $${Number(
        i.amount || 0
      ).toLocaleString()}`,
      date: i.created_date || i.createdAt || new Date(),
    })),

    ...customers.slice(0, 3).map((c) => ({
      type: "customer",
      title: `New customer: ${c.name || "Unknown"}`,
      subtitle: c.company || c.email || "No details",
      date: c.created_date || c.createdAt || new Date(),
    })),
    ...
    ...tasks.slice(0, 3).map((t) => ({
      ...
      date: t.created_date || t.createdAt || new Date(),
    })),
  ]
