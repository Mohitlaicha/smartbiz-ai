...
import { Plus, Search, MoreHorizontal, Mail, Phone, Building2, Users } from 'lucide-react';
...
const initialForm = { name: '', email: '', phone: '', company: '', status: 'lead', notes: '' };
...
export default function Customers() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['customers'] }); closeDialog(); },
  });
  ...
  const closeDialog = () => { setDialogOpen(false); setEditingCustomer(null); setForm(initialForm); };

  const openEdit = (customer) => {
    setEditingCustomer(customer);
    setForm({ name: customer.name, email: customer.email, phone: customer.phone || '', company: customer.company || '', status: customer.status || 'lead', notes: customer.notes || '' });
    setDialogOpen(true);
  };
      ...
      <div className="glass rounded-2xl p-6">
        ...
        {filtered.length === 0 && !isLoading ? (
          <EmptyState
            ...
            action={<Button onClick={() => setDialogOpen(true)} size="sm"><Plus className="w-4 h-4 mr-2" /> Add Customer</Button>}
          />
        ) : (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1 overflow-hidden">
                      ...
                      <p className="text-xs text-muted-foreground break-words">{customer.email}{customer.company && ` · ${customer.company}`}</p>
                      <div className="mt-1.5">
                        <Badge variant="outline" className={cn("text-xs capitalize", statusColors[customer.status])}>{customer.status}</Badge>
                      </div>
                    </div>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem className="text-destructive" onClick={() => deleteMutation.mutate(customer.id)}>Delete</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
                </motion.div>
      ...
      <Dialog open={dialogOpen} onOpenChange={closeDialog}>
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div><Label>Name *</Label><Input className="mt-1" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></div>
